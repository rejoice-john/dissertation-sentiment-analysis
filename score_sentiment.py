...
PROGRESS_EVERY = 500   # print a progress line every 500 comments while scoring
...
def print_label_breakdown(series, rows):
    # Prints each tool's positive/negative/neutral split for this series,
    # so the label distribution is visible as soon as scoring finishes
    # rather than only in the later dataset_overview output
    total = len(rows)
    for tool, label_field in [
        ...
        positive = sum(1 for r in rows if r[label_field] == 'positive')
        negative = sum(1 for r in rows if r[label_field] == 'negative')
        neutral = sum(1 for r in rows if r[label_field] == 'neutral')
        print('  ' + tool + ': ' + str(positive) + ' positive, ' + str(negative)
              + ' negative, ' + str(neutral) + ' neutral (of ' + str(total) + ')')
    ...
    with open(input_path, 'r', encoding='utf-8') as f:
        ...
        for i, row in enumerate(reader, start=1):
            ...
            if i % PROGRESS_EVERY == 0:
                print('  ...scored ' + str(i) + ' comments so far')
 
    print('  Scored ' + str(len(rows)) + ' comments for ' + series)
    print_label_breakdown(series, rows)
    return rows
...
print('Scoring comments for all series...')
print('')
